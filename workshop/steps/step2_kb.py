...
from config import CFG, PREFIX, REGION, get_state, save_state
...
def ensure_kb_role():
    ...
    perms = {
        "Version": "2012-10-17",
        "Statement": [
            {"Effect": "Allow", "Action": ["bedrock:InvokeModel"], "Resource": [CFG.embedding_model_arn]},
            {
                "Effect": "Allow",
                "Action": ["s3vectors:*"],
                "Resource": [
    ...
    iam.put_role_policy(RoleName=name, PolicyName=f"{PREFIX}-kb-perms", PolicyDocument=json.dumps(perms))
    return arn
...
def main():
    ...
    if ds_id:
        ...
        ds_id = ba.create_data_source(knowledgeBaseId=kb_id, name=ds_name, dataSourceConfiguration={"type": "CUSTOM"})[
            "dataSource"
        ]["dataSourceId"]
        print("   creada ✅")
